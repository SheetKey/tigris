...
-- | Holds the rendering information of trees generated by vox-hs.
newtype TreeModel = TreeModel RTree
instance Component TreeModel where
  type Storage TreeModel = Map TreeModel

-- | Holds the opengl buffers and shader program for tree rendering.
newtype GLTreeBuffers
  = GLTreeBuffers (GL.VertexArrayObject, GL.BufferObject, GL.BufferObject, GL.Program)
instance Component GLTreeBuffers where
  type Storage GLTreeBuffers = ReadOnly (TMVGlobal GLTreeBuffers)
